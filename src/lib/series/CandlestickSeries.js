...
function drawOnCanvas(ctx, props, moreProps) {
  ...
  const wickNest = Array.from(group(candleData, (d) => d.wick.stroke));
  ...
  candleNest.forEach((outer) => {
    const [strokeKey, strokeValues] = outer;
    if (strokeKey !== "none") {
      ctx.strokeStyle = strokeKey;
      ctx.lineWidth = candleStrokeWidth;
    }
    Array.from(strokeValues).forEach((inner) => {
      const [key, values] = inner;
      const fillStyle = values.width <= 1 ? key : hexToRGBA(key, opacity);
